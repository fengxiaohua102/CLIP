...
%     img_estimated = imgaussfilt(img_estimated,0.5); %    
end
...
function [img_estimated] = denoise_mixed(noisy,th)
    ...
    if(size(noisy,3)>1)
        noisy_3D = reshape(noisy,size(noisy,1),size(noisy,2),[]);
%         noisy_3D = prox_tv3d(noisy_3D,th/100);
        for K=1:size(noisy_3D,3)        
            [~,x] = BM3D(1, noisy_3D(:,:,K)./norm_const, th*255, 'lc', 0); % NLmeansfilter
            x(isnan(x)) = 0;   noisy_3D(:,:,K) = x;
            noisy_3D(:,:,K) = medfilt2(noisy_3D(:,:,K),[7,7]);
        end
        img_estimated = reshape(noisy_3D,size(noisy)) .* norm_const;
    else
        [~,x] = BM3D(1, noisy./norm_const, th*255, 'lc', 0); % NLmeansfilter
        x(isnan(x)) = 0;        noisy = x.*norm_const;
%         img_estimated = prox_tv(noisy, th);
        img_estimated = medfilt2(noisy,[7,7]);
%         img_estimated = noisy;
    end
...
function img = denoise_DnCNN(net, noisy,sigma_hat)
    if(size(noisy,3)>1)
        ...
        res = vl_simplenn(net, gpuArray(single(noisy)),[],[],'conserveMemory', true, 'mode', 'test');
        img = gather(gpuArray(noisy) - res(end).x ./1);
    end 
end
